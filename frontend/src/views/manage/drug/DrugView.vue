...
export default {
  ...
  methods: {
    dataInit (drugData) {
      // 维修信息
      if (drugData.repairCode !== null && drugData.repairCode !== '') {
        this.$get(`/cos/repair-info/detail/${drugData.repairCode}`).then((r) => {
          this.repairInfo = r.data
        })
      }
      // 预约信息
      this.$get(`/cos/reserve-info/detail/${drugData.drugCode}`).then((r) => {
